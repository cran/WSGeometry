...
#' @param C A real number specifying the unbalanced optimal transport parameter used in the Kantorovich-
#' Rubinstein distance. 
...
kr_bary<-function(data.list,C,method="fixed",support,wmaxIter,pmaxIter,return_type="default",thresh=10^-3,threads=1){
  ...
  if (method=="fixed"){
    run<-TRUE
    res<-umaaipm_fixed_wrap(lapply(lapply(data.list,"[[",1),t),lapply(data.list,"[[",2),support,C,wmaxIter,NULL,thresh,threads)
  ...
  if (method=="free"){
    run<-TRUE
    res<-umaaipm_free_wrap(lapply(lapply(data.list,"[[",1),t),lapply(data.list,"[[",2),support,C,wmaxIter,pmaxIter,NULL,thresh,threads)
    if (return_type=="default"){
  ...
  if (method=="multiscale"){
    run<-TRUE
    res<-kr_multi_scale_bary(lapply(lapply(data.list,"[[",1),t),lapply(data.list,"[[",2),C,support[1:2],support[3],support[4],wmaxIter,NULL,thresh,threads)
    if (return_type=="default"){
      return(list(positions=res[[1]],weights=res[[2]]))
    }
    if (return_type=="wpp"){
